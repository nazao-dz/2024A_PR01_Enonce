...
class Ghost:
    ...
    def move(self):
        ...
        if not self.dead:
            pass
                # Calculer la prochaine position en fonction de la direction et de la vitesse
        ...
        # Vérifier si la prochaine position entre en collision avec un mur
            if not self.check_collision(next_rect):
            # Mettre à jour la position du fantôme
                self.pos[0] = next_x
                self.pos[1] = next_y
                self.rect.topleft = self.pos  # Mettre à jour le rectangle

            else:
            # Changer la direction du fantôme s'il rencontre un mur
    ...
    def change_direction(self):
        ...
        random.shuffle(directions)  # Mélanger aléatoirement les directions

        for direction in directions:
        # Calculer la prochaine position du fantôme en fonction de la direction
            next_x = self.pos[0] + direction[0] * self.speed
            next_y = self.pos[1] + direction[1] * self.speed
            next_rect = pygame.Rect(next_x, next_y, GHOST_SIZE[0], GHOST_SIZE[1])
         # Vérifier si cette direction entraîne une collision avec un mur
        if not self.check_collision(next_rect):
            self.direction = direction  # Définir cette direction comme nouvelle direction
